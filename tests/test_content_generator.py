...
class TestGenerateBlogPost:
    ...
    @patch("src.content_generator.LLM_API_KEY", "test-key")
    @patch("src.content_generator.LLM_FALLBACK_API_KEY", "")
    @patch("src.content_generator.OpenAI")
    ...
    @patch("src.content_generator.time.sleep")
    @patch("src.content_generator.LLM_FALLBACK_API_KEY", "fallback-key")
    @patch("src.content_generator.LLM_FALLBACK_MODEL", "fallback-model")
    @patch("src.content_generator.LLM_FALLBACK_BASE_URL", "https://fallback.example.com/v1")
    @patch("src.content_generator.LLM_API_KEY", "test-key")
    @patch("src.content_generator.OpenAI")
    def test_fallback_on_rate_limit(self, mock_openai_cls, mock_sleep):
        ...
        fallback_post = BlogPost(
            ...
            cover_keywords="ai",
            introduction="Intro",
            sections=[BlogSection(heading="H", body="B")],
            conclusion="End",
            sources=[],
        )

        # Primary client raises RateLimitError
        mock_primary = MagicMock()
        mock_primary.beta.chat.completions.parse.side_effect = RateLimitError(
            message="rate limit", response=MagicMock(status_code=429), body=None,
        )

        # Fallback client succeeds
        mock_fb_message = MagicMock()
        mock_fb_message.parsed = fallback_post
        mock_fb_choice = MagicMock()
        mock_fb_choice.message = mock_fb_message
        mock_fb_response = MagicMock()
        mock_fb_response.choices = [mock_fb_choice]
        mock_fallback = MagicMock()
        mock_fallback.beta.chat.completions.parse.return_value = mock_fb_response

        # First call returns primary, second returns fallback
        mock_openai_cls.side_effect = [mock_primary, mock_fallback]
        ...
        result = generate_blog_post(items)
        assert result.title == "Fallback Post"
